...
- 채우는 시점은 성사 배치와 **프로필 저장** 두 곳이다. 성사 때 한 번 채우고, 그 뒤 설정 > 프로필에서 이름을 바꾸면 `FirestoreProfileRepository.save`가 자기 항목의 `displayName`을 다시 쓴다 — 상대 화면의 지출자 이름이 이 사본에서만 오기 때문이다. 쓰기는 커플 문서 `update` 권한(멤버면 허용)으로 통과한다.
  - 성사 배치에 넣는 이름도 구글 이름이 아니라 `users/{uid}.displayName`(비어 있으면 구글 이름)이다 — 내 항목은 수락자가 자기 문서에서, 상대 항목은 초대 문서의 `inviterName`에서 온다. 구글 이름을 쓰면 연결 **전에** 바꾼 이름이 상대에게 닿지 않는다.
...
| inviterName | String? | 수락 화면의 "○○님과 연결할까요?" 표시용. 발급 시점의 `users/{inviterUid}.displayName`(비어 있으면 구글 이름) |
